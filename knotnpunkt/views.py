...
def home():
    ...
    for a in ausleihen:
        ausleihe = {'empfaenger': a.empfaenger, "ts_von": a.ts_von,
                    "ts_bis": a.ts_bis, "materialien": a.materialien.split(",")}
        if a.ts_von > date.today():
            ausleihen_filtered_future.append(ausleihe)
        else:
            ausleihen_filtered_past.append(ausleihe)
    return render_template('home.html', apps=current_user.views(), ausleihen_zukunft=ausleihen_filtered_future, ausleihen_alt=ausleihen_filtered_past)
